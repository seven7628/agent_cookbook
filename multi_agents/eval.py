    ...
    messages = [
        SystemMessage(
            ...
            1. 当前步骤结果正确时:
                ...
                b. 当前步骤结果正确并且未能完成用户所提出的问题时，则评估结果为: continue。如果需要调整下一步计划工具调用信息, 则结合历史计划运行结果返回tool_params工具调用参数，同时将评估信息update_plan设置为True.
            2. 当前步骤结果不正确时:
                a. 当前步骤结果不正确并且可以通过调整计划完成用户问题时，评估结果为: restart。结合历史计划运行结果返回tool_params工具调用参数，同时将评估信息update_plan设置为True.
